...
const STUDENT = { nombre: '', apellido: '', edad: '', cedula: '', mama: '', papa: '', direccion: '', photo: null, discapacidad: '' };
  ...
  return [
    {
      name: 'Identidad', icon: '🪪', questions: [
        ...
        {
          L2: { text: 'Completa tu número de cédula', type: 'fill', answer: () => STUDENT.cedula },
          L3: { text: 'Escribe tu número de cédula', type: 'write', answer: () => STUDENT.cedula }
        }
      ]
    },
    {
...
$('btnStartGame').onclick = () => {
  // Validate all fields
  const fields = { nombre: 'regNombre', apellido: 'regApellido', edad: 'regEdad', cedula: 'regCedula', discapacidad: 'regDiscapacidad', mama: 'regMama', papa: 'regPapa', direccion: 'regDireccion' };
...
function getActiveSections() {
  ...
  return secs.map(sec => ({
    ...sec,
    questions: sec.questions.filter(q => q[`L${APP.level}`] !== undefined)
...
function totalQ() { return getActiveSections().reduce((s, sec) => s + sec.questions.length, 0) }
function curIdx() { let i = 0; const secs = getActiveSections(); for (let j = 0; j < APP.section; j++)i += secs[j].questions.length; return i + APP.question }
...
function loadQuestion() {
  ...
  if (qd.type === 'image-select') {
    ...
    html += `<input type="text" class="input-answer" id="ansInput" placeholder="${qd.type === 'fill' ? 'Escribe aquí tu respuesta' : 'Escribe tu respuesta completa'}" autocomplete="off">
      <div class="mic-row"><button id="btnMic" class="btn-mic" onclick="startVoice()" title="Responder con voz">🎤 Hablar</button></div>`;
...
window.startVoice = function () {
  ...
  if (!SpeechRec) { speak('Tu dispositivo no soporta reconocimiento de voz'); return; }
  if (_voiceRec) { _voiceRec.stop(); return; }
  const btn = $('btnMic'); if (!btn) return;
  btn.classList.add('mic-listening'); btn.textContent = '🔴 Escuchando...';
  _voiceRec = new SpeechRec();
  _voiceRec.lang = 'es-MX'; _voiceRec.interimResults = false; _voiceRec.maxAlternatives = 1;
  _voiceRec.onresult = e => {
    const text = e.results[0][0].transcript.trim();
    const inp = $('ansInput'); if (inp) inp.value = text;
    btn.classList.remove('mic-listening'); btn.textContent = '✅ ' + text;
    _voiceRec = null;
    setTimeout(() => $('btnSubmitAnswer')?.click(), 600);
  };
  _voiceRec.onerror = e => {
    btn.classList.remove('mic-listening'); btn.textContent = '🎤 Hablar'; _voiceRec = null;
    console.error('Error de Reconocimiento:', e);
    if (e.error === 'not-allowed') {
      speak('Acceso al micrófono denegado. Asegúrate de dar permisos y usar un servidor seguro o GitHub Pages.');
      alert('⚠️ Permiso de Micrófono Requerido:\n\n1. Asegúrate de dar permiso de micrófono a la página.\n2. IMPORTANTE: Los navegadores modernos bloquean el reconocimiento de voz si abres el archivo localmente como "file:///".\n\nPor favor, pruébalo subiendo el juego a GitHub Pages (que tiene HTTPS seguro) o usando un servidor web local.');
    } else {
      speak('No te escuché, intenta de nuevo');
    }
  };
  _voiceRec.onend = () => { btn.classList.remove('mic-listening'); if (_voiceRec) { _voiceRec = null; btn.textContent = '🎤 Hablar'; } };
  _voiceRec.start();
};
...
$('btnSubmitAnswer').onclick = () => {
  ...
  if (qd.type === 'image-select') {
    ...
    correct = (normVal === normExpected) || 
              (normVal.includes(normExpected) && normExpected.length > 1) || 
              (normExpected.includes(normVal) && normVal.length > 1);
  } else if (qd.type === 'order') {
    const slots = $$('#oTarget .order-slot');
    const expected = typeof qd.answer === 'function' ? qd.answer() : '';
...
function normalize(s) {
  ...
  // Convert common Spanish number words to digits
  const numWords = {
...
function showResults() {
  ...
  try {
    const applause = new Audio('https://actions.google.com/sounds/v1/human/applause_crowd_cheer.ogg');
    applause.volume = 0.6;
    applause.play().catch(e => console.log('Audio disabled/not ready'));
  } catch (e) {
  ...
  $('resultStats').innerHTML = `
    <div class="animated-id-card">
      ...
      <div class="id-body">
        <div class="id-photo">
          <img src="${STUDENT.photo || 'icon-192.png'}" alt="Foto">
        </div>
        <div class="id-details">
          <p><strong>NOMBRES:</strong> ${STUDENT.nombre} ${STUDENT.apellido}</p>
          <p><strong>CÉDULA:</strong> ${STUDENT.cedula}</p>
          <p><strong>EDAD:</strong> ${STUDENT.edad} años</p>
          <p><strong>REPRESENTANTES:</strong> ${STUDENT.mama} y ${STUDENT.papa}</p>
          <p><strong>DOMICILIO:</strong> ${STUDENT.direccion}</p>
          <p><strong>CONDICIÓN:</strong> ${STUDENT.discapacidad}</p>
        </div>
      </div>
      <div class="id-footer">
        <div class="signature">Válido para jugar</div>
      </div>
    </div>`;
  showScreen('screenResult');
  speak(`¡Felicidades ${STUDENT.nombre}! Completaste el juego. Aquí está tu nueva cédula oficial.`);
  spawnConfetti();
}
